...
Singleton {
  ...
  PwObjectTracker {
    objects: Pipewire.nodes.values.filter(pwNode => pwNode.audio && !pwNode.isStream)
  }
  ...
  function looksLikeSystemVirtualMic(node) {
    ...
    return ["cava", "monitor", "system"].some(s => combined.includes(s));
  }
  ...
  function looksLikeScreencast(node) {
    ...
    return ["xdg-desktop-portal", "xdpw", "screencast", "screen", "gnome shell", "kwin", "obs"].some(s => combined.includes(s));
  }

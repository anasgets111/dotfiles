...
Item {
    ...
    function updateWorkspaces(arr) {
        // annotate populated = has at least one window
        arr.forEach(function(w) {
            w.populated = (w.active_window_id !== null)
        })
        // sort by idx and replace
        arr.sort(function(a,b){ return a.idx - b.idx })
        workspaces = arr

        // animate if the focused idx changed
    ...
    function updateSingleFocus(id) {
        ...
        // toggle is_focused + is_active, leave `populated` alone
            ...
            onRead: function(segment) {
                ...
                if (evt.WorkspacesChanged) {
                    root.updateWorkspaces(
                        evt.WorkspacesChanged.workspaces
                    )
                } else if (evt.WorkspaceActivated) {
                    root.updateSingleFocus(
                        evt.WorkspaceActivated.id
                    )
                }
            }
        }
    ...
    MouseArea {
        ...
        onPositionChanged: function(m) {
            ...
            hoveredIndex = (idx >= 1 && idx <= workspaces.length)
                         ? idx
                         : 0
        }
        onClicked: {
            if (hoveredIndex > 0 &&
                hoveredIndex !== currentWorkspace)
    ...
    // collapsed slide animation (unchanged)…
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            color: workspaceColor({
                idx: slideFrom, is_focused: true, populated: true
            })
        ...
        Rectangle {
            ...
            radius: Theme.itemRadius
            color: workspaceColor({
                idx: slideTo, is_focused: true, populated: true
            })
            x: (slideProgress - 1) *

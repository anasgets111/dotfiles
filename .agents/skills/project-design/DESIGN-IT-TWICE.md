...
*   **Code Sketch:** A rough Lua snippet proving the constraints. *Not a proposal, just a baseline.*
...
*   **Agent 2 (The Flexible Extender):** Support multiple use cases and future extensions (e.g., a handler table keyed by kind).
*   **Agent 3 (The Pragmatic Config Author):** Optimize for the 80% common caller. Make the default case a trivial one-liner (e.g., one `require` and one call, like `compositor.detach("logout")`).
*   **Agent 4 (The Ports & Adapters Purist):** (If applicable) Design strict seams across processes (capabilities, subprocesses).

**Sub-Agent Output Requirements:**
1.  **Interface:** LuaLS annotations (`---@param`, `---@return`), error modes, invariants.

-- Empty-list state. A bare dim `cell` reads as a load failure, not an intentional empty state.
--
-- The caller supplies `visible`; `lib/util.lua`'s `shown_when` maps the capability payload to it.
--
-- With `opts.icon`, use a large dim glyph over the message and a 120px minimum height, which reads
-- as state rather than a gap. Without it, use the single line from the launcher and power menu.
...
return function(message, visible, opts)
    ...
    if opts.subtext then
        -- The third line explains whether nothing arrived or something is suppressing the list,
        -- rather than repeating the message. Use a colour at that alpha because `cell` takes no
        -- node `opacity`.

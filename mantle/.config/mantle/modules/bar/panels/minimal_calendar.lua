-- Current month grid with today marked.
--
...
--
-- Use `os.date`/`os.time`, with `mantle.system.time` supplying *today*. There is no calendar
-- capability, and none should be added.
--
-- Not a panel. It sits in the clock's hover tooltip; a click opens the notifications panel, so the
-- always-visible bar readout does not open a month grid.
...
local title = cell(mantle.system:map(function(s)
    return { { text = os.date("%B %Y", (s and s.time) or os.time()), bold = true } }
end), theme.FG, theme.font.sm, { width = "Fill", align = "Center" })

-- A popup surface's size is explicit, so its host cannot measure this like a `Column`. Height
-- follows row count: a five-week month is one `DAY_SIDE` shorter than a six-week one, and surface
-- size takes `integer|Bound`.
--

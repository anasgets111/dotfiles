...
-- weather reading leads once there is one; `weather.code` holds `-1` until then.
local clock = cell(util.bold(computed({ mantle.system, weather.code, weather.temperature },
    function(system, code, celsius)
        local shown = os.date("%a %d %b  %I:%M %p", system and system.time)
        if code >= 0 then
...
local clock_tooltip = tooltip({
    ...
    height = calendar.height:map(function(grid)
        -- Date, time and the two weather lines, each `sm` over `xs`, with a gap after each of the four.
        return grid + (DATE_LINE + TIME_LINE) * 2 + theme.spacing.md * 2 + theme.spacing.xs * 4
    end),
    children = {
        ...
        cell(computed({ weather.code, weather.location }, function(code, where)
            ...
            local place = where.place_name or ""
            return {

...
--- @param player table? one entry of `mantle.mpris`'s `players`
--- @return boolean
...
--- Video classification from one `mantle.mpris` payload. Pure and payload-based so `lib/idle.lua`
--- can use the value from `on_change` instead of a possibly stale `computed` in its callback.
--- @param m table? `mantle.mpris`'s payload
--- @return boolean

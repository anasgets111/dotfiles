// A straight edge sweeping across.
//
// The engine has already fitted both pictures and hands them back through `mantle_from`/
// `mantle_to`, so no fill-mode sampling is needed here.
//
...
void main() {
    vec4 from = mantle_from(v_uv);
    vec4 to = mantle_to(v_uv);

    // Non-linear: most of the useful range is at the low end.
